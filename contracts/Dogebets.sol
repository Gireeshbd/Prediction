...
contract CandleGeniePrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    function _safeStartRound(
        uint256 epoch,
        uint256 houseBetBullAmount,
        uint256 houseBetBearAmount
    ) internal {
    ...
    function _safeClaim(uint256[] calldata epochs) internal {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (claimable(epochs[i], msg.sender)) {
                ...
                uint256 rewardAmount = (Bets[epochs[i]][msg.sender].amount * round.rewardAmount) /
    ...
    function RoundExecute(
        ...
        require(
            startOnce && lockOnce,
            "Can only execute round after startRound and lockRound is triggered"
        );
        require(
            Rounds[currentEpoch - 2].closeTimestamp != 0,
            "Can only execute round after previous round started"
        );
        require(
            block.timestamp >= Rounds[currentEpoch - 2].closeTimestamp,
            "Can only execute round round after previous round ended"
        );
        require(
            block.timestamp >= Rounds[currentEpoch].lockTimestamp,
            "Can only execute round after lock timestamp reached"
        );
        require(
            block.timestamp <= Rounds[currentEpoch].closeTimestamp,
            "Can only execute round before current close timestamp"
        );
    ...
    function ClaimReferred(uint256[] calldata epochs, address referrer)
        external
        nonReentrant
        notContract
    {
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        ...
        if (
            ...
            block.timestamp < round.closeTimestamp + 5
        ) {
            return false;
        }

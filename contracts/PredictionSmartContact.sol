...
contract SilverlinePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public BullAmount;
    uint256 public BearAmount;
    ...
    event HouseBetUpdated(uint256 betAmount);
    event HouseBetMade(address addr, uint256 Epoch, uint256 _BullAmount, uint256 _BearAmount);
    ...
    function betBear(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        // Update user data
        BetInfo memory betInfo = ledger[epoch][msg.sender];
    ...
    /**
     * @notice claim SLNrewards for lost Round
     * @param epochs: array of epochs
     */

    function claimSLNrewards(uint256 epochs) external nonReentrant notContract {}
    ...
    /**
     *@notice bets from the house side
     *@dev only callable by excecute Function
    ...
    function _safeHouseBet() internal {
        ...
        if (BullAmount > 0) {
            ...
            BetInfo memory betInfo = ledger[currentEpoch][address(this)];
            betInfo.position = Position.Bull;
    ...
    function HouseBet() external onlyAdminOrOperator whenNotPaused notContract {
    ...
    function setHouseBetAmount(uint256 _betAmount) external onlyAdminOrOperator {
        BullAmount = _betAmount;
        BearAmount = _betAmount;

        emit HouseBetUpdated(_betAmount);
    }
     ...
     */
    function claimTreasury(uint256 _treasuryAmount) external nonReentrant onlyAdmin {
        // uint256 currentTreasuryAmount = treasuryAmount;
        // treasuryAmount = 0;
        _safeTransferBNB(adminAddress, _treasuryAmount);

        emit TreasuryClaim(_treasuryAmount);
    }

    /**
     *@notice reading the treasury Fund
     *@dev
     */
    function _TreasuryAmount() external view returns (uint256) {
    ...
    /**
     * @notice Transfering BNB to the contract
     * @dev
     */
